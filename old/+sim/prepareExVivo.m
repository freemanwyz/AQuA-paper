function p = prepareExVivo(datOrg)
    % need to be tuned for each data
    
    sz = size(datOrg);
    
    % create dynamic patterns -------
    datV = reshape(datOrg,[],sz(3));
    xm = mean(datV,1);
    % figure;plot(xm);
    ...
    for nn=1:nTmplt
        tx = pkLst(nn);
        trg = max(tx-10,1):min(tx+10,sz(3));
        dBurst = datOrg(:,:,trg);
    ...
    p.sz = size(dAvg);
    p.dStd = dStd;
    p.nTmplt = nTmplt;  % dynamic pattern numbers
    p.sucRtAvg = sucRtAvg;  % overall success rate per pixel
    p.fg = fg;
    p.dAvg = dAvg;
    p.dfBurstAvgLst = dfBurstAvgLst;
    p.dBurstStdLst = dBurstStdLst;   
    
    % time and temporal filter
    p.tf = 120;
    p.tfUp = 20;
    p.tfDn = 100;
    p.temporalCross = 2;
