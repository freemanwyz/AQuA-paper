function [vox1,htMap0,fiu0] = refineRegion(df0,e0,nn,stdEst,delta,osTb,mthdSolver,mthdZ)
    ...
    if mthdZ==0  % find local maximum as seeds
        ...
        for ii=1:numel(seedIdx)
            ...
            zMap0(pixSel) = zMap0(pixSel) + getFisherTrans(r0Vec,size(e0,3));
        end
        zMap0(cntMap==0) = nan;
        cntMap(cntMap==0) = 1;
        zMap0 = zMap0./cntMap;  % take average
    end
    if mthdZ==1  % use all pixels, ignore propagation
        zMap0 = nan(size(fiu0));
        pixSel = find(fiu0Di>0);
        charx0 = zscore(mean(d0Vec(pixSel,:),1),0,2);
        d0Sel = zscore(d0Vec(pixSel,:),0,2);
        r0Vec = mean(d0Sel.*charx0,2);
    ...
    if mthdSolver==1  % Matlab
        try
        [htMap00,~] = htrg2(zMap0,fiu0Er,vMap0,osTb,2,4,4,4);
        catch
            keyboard
        end
    end
    if mthdSolver==2  % Matlab, do not specify seed
        [htMap00,~] = htrg2(zMap0,[],vMap0,osTb,2,4,4,4);
    end
