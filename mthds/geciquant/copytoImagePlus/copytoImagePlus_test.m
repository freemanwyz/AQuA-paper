classdef copytoImagePlus_test < matlab.unittest.TestCase
    %copytoImagePlus_test < matlab.unittest.TestCase
    %
    % clear;close all;clc;testCase = copytoImagePlus_test;res = testCase.run;disp(res);
    %
    %
    %
    %
    ...
    methods (Test)
        function test1(testCase)
            % clear;close all;clc;testCase = copytoImagePlus_test;
            ...
            imp = copytoImagePlus(I4cz)
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;1]);
            imp.close();

            imp = copytoImagePlus(I4cz,'YXCZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;1]);
            imp.close();
            
            imp = copytoImagePlus(I4cz,'YXCZT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;1]);
            imp.close();

            I4zc = permute(I4cz,[1 2 4 3]);

            imp = copytoImagePlus(I4zc,'YXZC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;1]);
            imp.close();
            
            imp = copytoImagePlus(I4zc,'YXZCT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;1]);
            imp.close();           
            ...
            imp = copytoImagePlus(I4ct,'YXCT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;51]);
            imp.close();    
            
            imp = copytoImagePlus(I4ct,'YXCTZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;51]);
            imp.close(); 
            
            I4tc = permute(I4ct,[1 2 4 3]);
            
            imp = copytoImagePlus(I4tc,'YXTC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;51]);
            imp.close();             
            
            imp = copytoImagePlus(I4tc,'YXTCZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;51]);
            imp.close();   
            ...
            imp = copytoImagePlus(I4zt,'YXZT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;51]);
            imp.close();
            
            imp =  copytoImagePlus(I4zt,'YXZTC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;51]);
            imp.close();
            
            I4tz = permute(I4zt,[1 2 4 3]);
            
            imp = copytoImagePlus(I4tz,'YXTZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;51]);
            imp.close();
            
            imp = copytoImagePlus(I4tz,'YXTZC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;51]);
            imp.close();
            ...
            imp = copytoImagePlus(I3c)
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;1]);
            imp.close();
            
            imp = copytoImagePlus(I3c,'YXC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;1]);
            imp.close();
            
            imp = copytoImagePlus(I3c,'YXCZT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;1]);
            imp.close();
            
            imp = copytoImagePlus(I3c,'YXCTZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;1;1]);
            imp.close();
            
            I3z = squeeze(I16(:,:,1,:,1));
            
            imp = copytoImagePlus(I3z,'YXZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;1]);
            imp.close();            
            
            imp = copytoImagePlus(I3z,'YXZCT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;1]);
            imp.close();
            
            imp = copytoImagePlus(I3z,'YXZTC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;5;1]);
            imp.close();
            
            I3t = squeeze(I16(:,:,1,3,:));
            
            imp = copytoImagePlus(I3t,'YXT')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;1;51]);
            imp.close();  
            
            imp = copytoImagePlus(I3t,'YXTCZ')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;1;51]);
            imp.close();
            
            imp = copytoImagePlus(I3t,'YXTZC')
            testCase.verifyEqual(double(imp.getDimensions), [171;196;1;1;51]);
            imp.close();       
            
            %% verify the interger values for uint8
            
            imp = copytoImagePlus(I8)
            testCase.verifyEqual(double(imp.getDimensions), [171;196;2;5;51]);
